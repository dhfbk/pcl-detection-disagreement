...
class MachampClassifier(Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.metric == "acc":
            ...
            #self.metrics = {"macro-f1": FBetaMeasure()}
            self.metrics = {"macro-f1": FBetaMeasure(labels=[self.vocab.get_token_index("1", namespace=task)])}
        elif self.metric == "macro-f1-all":
            self.metrics = {"macro-f1-all": FBetaMeasure(average='macro')}
        else:
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        for metric_name, metric in self.metrics.items():
            if metric_name.endswith('macro-f1-all'):
                if metric._true_positive_sum == None:
                    ...
                    #     main_metrics[f".run/{self.task}/{metric_name}"] = {'precision':metric.get_metric(reset)["precision"][0], 'recall':metric.get_metric(reset)["recall"][0], 'fscore':metric.get_metric(reset)["fscore"][0]}
                    # else:
                    #     main_metrics[f".run/{self.task}/{metric_name}"] = metric.get_metric(reset)
                    #print(metric_name)
                    #print(metric._true_positive_sum)
                    main_metrics[f".run/{self.task}/{metric_name}"] = metric.get_metric(reset)

            elif metric_name.endswith('macro-f1'):
                if metric._true_positive_sum == None:
                    main_metrics[f".run/{self.task}/macro-f1"] = 0.0
                else:
                    main_metrics[f".run/{self.task}/macro-f1"] = metric.get_metric(reset)["fscore"][0]
            else:
                main_metrics[f".run/{self.task}/{metric_name}"] = metric.get_metric(reset)
        return {**main_metrics}
